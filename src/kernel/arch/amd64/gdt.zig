...
fn wrietTSS(ss0: u32, esp0: u32) void {
    const base: usize = @intFromPtr(&tss);
    var low = &gdt[gdt.len - 2];
    const high = &gdt[gdt.len - 1];

    low.base_low = @truncate(base);
    low.base_mid = @truncate(base >> 16);
    low.base_high = @truncate(base >> 24);

    low.limit = @sizeOf(TSS) - 1;
    low.flags = 0b1;
    low.access = 0b1001;

    high.* = @bitCast(base & 0xFFFF_FFFF_0000_0000);
